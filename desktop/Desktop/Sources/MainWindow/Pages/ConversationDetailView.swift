...
struct ConversationDetailView: View {
            ...
            VStack(alignment: .leading, spacing: 0) {
                ...
                ScrollView {
                    ...
                    VStack(alignment: .leading, spacing: 0) {
                        ...
                        .padding(.vertical, 10)
                        .background(VibeAIColors.backgroundTertiary.opacity(0.4))
                    ...
                    .background(
                        RoundedRectangle(cornerRadius: 16)
                            .fill(VibeAIColors.backgroundSecondary.opacity(0.6))
                    )
    ...
    private var viewTranscriptButton: some View {
        Button(action: {
            ...
            .foregroundColor(showTranscriptDrawer ? .white : VibeAIColors.textSecondary)
            .padding(.horizontal, 12)
            .padding(.vertical, 6)
            .background(
                Capsule()
                    .fill(showTranscriptDrawer ? VibeAIColors.purplePrimary : VibeAIColors.backgroundTertiary)
            )
        }
        .buttonStyle(.plain)
    }
            ...
            if !folders.isEmpty {
                Menu {
                    ...
                    Image(systemName: displayConversation.folderId != nil ? "folder.fill" : "folder")
                        .scaledFont(size: 14)
                        .foregroundColor(displayConversation.folderId != nil ? VibeAIColors.purplePrimary : VibeAIColors.textSecondary)
                        .frame(width: 28, height: 28)
                        .background(
                            Circle()
                                .fill(VibeAIColors.backgroundTertiary)
    ...
    private var statusColor: Color {
        ...
        case .completed:
            return VibeAIColors.success
        case .processing, .merging:
            return VibeAIColors.info
        case .inProgress:
            return VibeAIColors.warning
        case .failed:
                ...
                ForEach(activeItems) { item in
                    HStack(alignment: .top, spacing: 10) {
                        Image(systemName: item.completed ? "checkmark.circle.fill" : "circle")
                            .scaledFont(size: 16)
                            .foregroundColor(item.completed ? VibeAIColors.success : VibeAIColors.textTertiary)

                        Text(item.description)
                            .scaledFont(size: 14)
                            .foregroundColor(item.completed ? VibeAIColors.textTertiary : VibeAIColors.textPrimary)
                            .textSelection(.enabled)
                            .strikethrough(item.completed, color: VibeAIColors.textTertiary)
                    }
                    .padding(12)
                    .frame(maxWidth: .infinity, alignment: .leading)
                    .background(
...
struct SuggestedAppCard: View {
    ...
    var body: some View {
        Button(action: onTap) {
            ...
            .background(
                RoundedRectangle(cornerRadius: 12)
                    .fill(isHovering ? VibeAIColors.backgroundTertiary : VibeAIColors.backgroundSecondary)
            )
        }
        .buttonStyle(.plain)
        .disabled(isLoading)
...
struct AppSelectorRow: View {
    ...
    var body: some View {
        Button(action: onSelect) {
            ...
            .background(
                RoundedRectangle(cornerRadius: 10)
                    .fill(isSelected || isHovering ? VibeAIColors.backgroundTertiary : Color.clear)
            )
        }
        .buttonStyle(.plain)
        .disabled(isLoading)
